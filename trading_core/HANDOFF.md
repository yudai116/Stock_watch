...
**ライブ運用ランナーは実装済み**（`execution/live_runner.py`、運用手順 =
`OPERATIONS.md`）: 日次1コマンドで 取込→約定→レジーム判定→注文→帳簿記録。
キルスイッチ(-23.5%)・冪等性・ペーパー帳簿(SQLite)・Saxo SIM差し込み口込み。

**ペーパートレードは 2026-07-08 稼働開始済み**（ユーザーPCのタスクスケジューラ
"trading_core_paper" 毎朝7:00 JST。初回判定 bull→buy を確認済み）。
帳簿 live_state.sqlite はユーザーPCのみに存在（クラウド監視には push が必要）。

**敵対的レビュー実施済み（2026-07-09、GO-with-fixes）**:
`reports/adversarial_review_2026-07.md` 参照。M2(VIXラグ)/M4(DSR N固定)は
修正済み、公式値 v1.1 = OOS Sharpe 0.85 / CAGR 18.1% / DD -18.8%、
ホールドアウト 0.95 / 14.8% / -11.8%、DSR 0.805(N=6)不合格。
重要な認識: エッジの実体は2022年ベア回避の1事象。「パラメータ0個」は撤回し
「埋め込み定数2個・感度測定済み・保守側」。**G-1判定はレジーム転換を
1回経験するまで完了扱いにしない**（OPERATIONS.md に明記）。

残課題（優先順）: (1) whipsawストレステスト（2011/2015型チョッピー相場の
合成/歴史検証）、(2) 実弾前の Saxo 実約定照合ロジック、(3) ゲート指標の
連続窓ベース統一。ホールドアウトは開封済みのため再利用禁止。
